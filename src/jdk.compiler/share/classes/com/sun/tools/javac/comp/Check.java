 ...
 **************************************************************************/
    ...
    //where
        private boolean withinAnonConstr(Env<AttrContext> env) {
            return (env.enclClass.name.length() == 0) &&
 ...
 **************************************************************************/
    ...
    private Set<Name> getDefaultTargetSet() {
        if (defaultTargets == null) {
            defaultTargets = Collections.unmodifiableSet(new HashSet<>(Arrays.asList(defaultTargetMetaInfo())));
        }
 ...
 **************************************************************************/
    ...
    private class SerialTypeVisitor extends ElementKindVisitor14<Void, JCClassDecl> {
        ...
        private static final Set<String> serialMethodNames =
            Collections.unmodifiableSet(new HashSet<>(Arrays.asList("writeObject", "writeReplace", "readObject", "readObjectNoData", "readResolve")));

        private static final Set<String> serialFieldNames =
            Collections.unmodifiableSet(new HashSet<>(Arrays.asList("serialVersionUID", "serialPersistentFields")));
