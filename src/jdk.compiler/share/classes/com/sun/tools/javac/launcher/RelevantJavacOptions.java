...
record RelevantJavacOptions(List<String> forProgramCompilation,
    ...
    static RelevantJavacOptions of(ProgramDescriptor program, String... runtimeArgs) throws Fault {
        ...
        if (sourceOpt != null) {
            ...
            programOptions.addAll(Collections.unmodifiableList(Arrays.asList("--release", sourceOpt)));
            subsequentOptions.addAll(Collections.unmodifiableList(Arrays.asList("--release", sourceOpt)));
        }
        ...
        for (int i = 0; i < runtimeArgs.length; i++) {
            ...
            switch (opt) {
                ...
                case "--enable-preview" -> {
                    ...
                    subsequentOptions.add(opt);
                    if (sourceOpt == null) {
                        String feature = String.valueOf("1.8".equals(System.getProperty("java.specification.version")) ? 8 : Integer.parseInt(System.getProperty("java.specification.version")));
                        programOptions.addAll(Collections.unmodifiableList(Arrays.asList("--release", feature)));
                        subsequentOptions.addAll(Collections.unmodifiableList(Arrays.asList("--release", feature)));
                    }
                }
                default -> {
        ...
        return new RelevantJavacOptions(Collections.unmodifiableList(new ArrayList<>(programOptions)), Collections.unmodifiableList(new ArrayList<>(subsequentOptions)));
    }

    private static List<String> computeListOfAddModules(ProgramDescriptor program, String value) {
        var modules = new ArrayList<>(Collections.unmodifiableList(Arrays.asList(value.split(","))));
