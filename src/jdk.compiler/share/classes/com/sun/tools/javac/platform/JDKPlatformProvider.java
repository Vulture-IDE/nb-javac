        ...
        if (Files.exists(ctSymFile)) {
            try (FileSystem fs = VMWrapper.pathFs(ctSymFile);
                 DirectoryStream<Path> dir =
                         VMWrapper.newDirectoryStream(fs.getRootDirectories().iterator().next())) {
        ...
        public JavaFileManager getFileManager() {
            ...
            if (Files.exists(file)) {
                try {
                    ...
                    if (fs == null) {
                        ctSym2FileSystem.put(file, fs = VMWrapper.pathFs(file));
                    }
                    ...
                    if (!hasModules) {
                        ...
                        try (DirectoryStream<Path> dir = VMWrapper.newDirectoryStream(root)) {
                            for (Path section : dir) {
                                if (section.getFileName().toString().contains(ctSymVersion) &&
                                    !section.getFileName().toString().contains("-")) {
                                    try (DirectoryStream<Path> modules = VMWrapper.newDirectoryStream(section)) {
