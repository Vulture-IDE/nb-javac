 ...
 **********************************************************************/
    ...
    void writeInnerClasses() {
        ...
        for (ClassSymbol inner : poolWriter.innerClasses) {
            ...
            databuf.appendChar(
                inner.owner.kind == TYP && !(inner.name.length() == 0) ? poolWriter.putClass((ClassSymbol)inner.owner) : 0);
            databuf.appendChar(
                !(inner.name.length() == 0) ? poolWriter.putName(inner.name) : 0);
            databuf.appendChar(flags);
        }
        endAttr(alenIdx);
    }
    ...
    void writeBootstrapMethods() {
        ...
        do {
            ...
            for (BsmKey bsmKey : Collections.unmodifiableList(new ArrayList<>(poolWriter.bootstrapMethods.keySet()))) {
